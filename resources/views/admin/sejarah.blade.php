  ...
  <script>
    ...
    function _submit(f){
      ...
      $.ajax({
        ...
        type: 'post',
        url: url('content/sejarah/'+uid),
        data: formData,
        contentType: false,
        processData: false,
        success: function(res){
          toast('Sejarah berhasil diperbarui');
          goTo('reload')
        }, error: function(err){
    ...
    function _edit(data){
      mod.find('.modal-title').html('Edit Sejarah')
      isNew = false; uid = data.id;
      $('iframe').contents().find('.wysihtml5-editor').html(data.sejarah);
